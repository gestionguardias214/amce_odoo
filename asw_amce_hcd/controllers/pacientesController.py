...
class PacientesController(Controller):
    ...
    def pacientes_sincronizar(self, **kw):
        historias_clinicas = (
            request.env["asw.historia"]
            .sudo()
            .search(
                [("historia_proceso_paciente", "=", False), ("hpa_dni", "!=", False)],
                order="id",
            )
        )
    ...
    def firmas_sincronizar(self, **kw):
        domain_firma_paciente = [
            ("historia_archivo_firma_pac_acompanante", "!=", False),
            ("historia_firma_pac_acompanante", "=", False),
        ]
        historias_clinicas = (
            request.env["asw.historia"].sudo().search(domain_firma_paciente, order="id")
        )
        tot = len(historias_clinicas)
        cnt = 0
