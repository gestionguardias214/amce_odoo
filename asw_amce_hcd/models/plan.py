...
class asw_plan(models.Model):
    ...
    plan_nombre = fields.Char(
        string = 'Nombre'
    ) 

    plan_codigo = fields.Integer(
        string = u'Código'
    )
    ...
    def create(self, values):
        if 'plan_codigo' in values and values['plan_codigo'] not in [False, '']:
            cnt = self.env['asw.plan'].search_count([('plan_codigo', '=', values['plan_codigo'])])
            if cnt > 0:
        ...
        if 'plan_nombre' in values and values['plan_nombre'] not in [False, '']:
            cnt = self.env['asw.plan'].search_count([('plan_nombre', '=', values['plan_nombre'])])
            if cnt > 0:
