...
class asw_mco(models.Model):
    ...
    mco_nombre = fields.Char(string="Motivo")

    mco_codigo = fields.Integer(string="Código")
    ...
    def create(self, values):
        if "mco_codigo" in values and values["mco_codigo"] not in [False, ""]:
            cnt = self.env["asw.mco"].search_count(
                [("mco_codigo", "=", values["mco_codigo"])]
            )
            if cnt > 0:
