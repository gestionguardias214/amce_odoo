...
class asw_al_llegar(models.Model):
    ...
    def create(self, values):
        if 'al_llegar_nombre' in values and values['al_llegar_nombre'] not in [False, '']:
            cnt = self.env['asw.al_llegar'].search_count([('al_llegar_nombre', '=', values['al_llegar_nombre'])])
            if cnt > 0:
