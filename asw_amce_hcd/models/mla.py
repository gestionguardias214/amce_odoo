...
class asw_mla(models.Model):
    ...
    mla_nombre = fields.Char(string="Motivo")

    mla_codigo = fields.Integer(string="Código")

    mla_color = fields.Char(string="Color")
    ...
    def create(self, values):
        if "mla_codigo" in values and values["mla_codigo"] not in [False, ""]:
            cnt = self.env["asw.mla"].search_count(
                [("mla_codigo", "=", values["mla_codigo"])]
            )
            if cnt > 0:
