...
class asw_trauma_tipo(models.Model):
    ...
    trauma_tipo_nombre = fields.Char(string="Nombre")

    active = fields.Boolean(string="Está activo?", default=True)

    @api.model
    def create(self, values):
        if "trauma_tipo_nombre" in values and values["trauma_tipo_nombre"] not in [
            False,
            "",
        ]:

            cnt = self.env["asw.trauma_tipo"].search_count(
                [("trauma_tipo_nombre", "=", values["trauma_tipo_nombre"])]
            )
            if cnt > 0:
