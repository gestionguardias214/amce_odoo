...
class asw_piel_mucosa(models.Model):
    ...
    piel_mucosa_nombre = fields.Char(string="Piel y mucosa")

    active = fields.Boolean(string="Está activo", default=True)

    @api.model
    def create(self, values):
        if "piel_mucosa_nombre" in values and values["piel_mucosa_nombre"] not in [
            False,
            "",
        ]:
            cnt = self.env["asw.piel_mucosa"].search_count(
                [("piel_mucosa_nombre", "=", values["piel_mucosa_nombre"])]
            )
            if cnt > 0:
                raise exceptions.Warning(
                    """Ya existe un registro para el nombre provisto. Por favor
