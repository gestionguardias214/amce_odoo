...
class asw_medicacion(models.Model):
    ...
    medicacion_nombre = fields.Char(
        string = 'Medicacion'
    )

    medicacion_codigo = fields.Integer(
        string = u'Código'
    )
    ...
    def create(self, values):
        if 'medicacion_codigo' in values:
            if values['medicacion_codigo'] not in [False, '']:
                cnt = self.env['asw.medicacion'].search_count([('medicacion_codigo', '=', values['medicacion_codigo'])])
                if cnt > 0:
        ...
        if 'medicacion_nombre' in values and values['medicacion_nombre'] not in [False, '']:
            cnt = self.env['asw.medicacion'].search_count([('medicacion_nombre', '=', values['medicacion_nombre'])])
            if cnt > 0:
                raise exceptions.Warning('''Ya existe un registro con la medicacion especificada. Por favor revise nuevamente
                y vuelva a intentarlo.''')
