...
class asw_fin_guardia(models.TransientModel):
    ...
    fin_fechahora = fields.Datetime(
        string="Fecha y hora finalizacion",
    ...
    def compute_cant_horas(self):
        if self.fin_fechahora != False and self.fin_fechahora_inicio != False:
            self.fin_cantidad_horas = self.calcular_horas(
                self.fin_fechahora, self.fin_fechahora_inicio
            )
    ...
    def _compute_obs(self):
        if self.fin_profesional.id != False:
            guardia = self.env["asw.guardia"].search(
                [("state", "=", "a"), ("gua_profesional", "=", self.fin_profesional.id)]
            )
    ...
    def actualizar_fin(self):
        ...
        periodo = self.obtenerPeriodo(
            self.fin_fechahora, self.fin_profesional.pro_grupo
        )

        if periodo.id == False:
            raise exceptions.ValidationError(
                "No existe un periodo configurado para guardar la guardia, por favor revise los periodos cargados."
            )
