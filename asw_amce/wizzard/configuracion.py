...
class asw_configuracion(models.Model):
    ...
    con_concepto = fields.Many2one(
        string=u'Concepto Presentismo',
        comodel_name='asw.concepto',
        ondelete='restrict',
    )

    con_concepto_sac = fields.Many2one(
        string=u'Concepto de SAC',
        comodel_name='asw.concepto',
        ondelete='restrict',
    )    
    
    con_minutos_tolerancia = fields.Integer(
    ...
    con_concepto_vianda = fields.Many2one(
        string=u'Concepto de Vianda',
        comodel_name='asw.concepto',
        ondelete='restrict',
    )

    con_concepto_viatico = fields.Many2one(
        string=u'Concepto de Viatico',
        comodel_name='asw.concepto',
        ondelete='restrict',
    )
    
    con_importe_vianda = fields.Float(
        string=u'Importe de Viandas',
    )
