...
class asw_relacionar_cateogira(models.TransientModel):
    ...
    cat_id = fields.Many2one(
        string="Categoria", comodel_name="asw.categoria", ondelete="restrict"
    )
    grupo_id = fields.Many2one(
        string="Grupo", comodel_name="asw.grupo", ondelete="restrict"
    )
    ...
    prof_ids = fields.Many2many(
        "asw.profesionales",
        "prof_categorias_wizzard",
        "wiz_id",
        "module_id",
    ...
    def relacionar_profesionales(self):
        self.cat_id.write({"cat_profesional": [(6, 0, self.prof_ids.ids)]})
