...
class asw_inicio_guardia(models.TransientModel):
    ...
    proc_tolerancia = fields.Integer(
        string=u'Minutos de Tolerancia',
    )
    ...
    def default_get(self, vals):
        ...
        tolerancia = self.env['asw.configuracion'].search([], limit=1).con_minutos_tolerancia or 0
        ...
        for record in self:
            ...
            for id in record.proc_profesional_concepto.ids:
                ...
                prof_con.write({
                    ...
                    'prc_descripcion': "%s - %s %s" % (prof_con.prc_concepto.name, guardia.id, record.proc_fechahora),
                })
    ...
    def _compute_precioxhora(self):
        for record in self:
            record.proc_precio_categoria = record.proc_categoria._compute_buscar_precio(record.proc_fechahora)
