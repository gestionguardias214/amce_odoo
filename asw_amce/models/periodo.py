...
class asw_periodo(models.Model):
    ...
    _sql_constraints = [
    ('codigo_unique', 'UNIQUE(per_codigo)', 'El codigo ya esta usado en otro campo, por favor elija un codigo diferente'),   
    ]
    ...
    def checkeo(self):
        if len( self.env['asw.calculo_sac'].search( [('sac_periodo', '=', self.id)] ) ) > 0:
            raise ValidationError("No se puede eliminar este periodo ya que posee uno o mas calculos de SAC relacionados. Elimine estas relaciones para borrar el periodo.")

        elif len(self.per_guardia) > 0:
            raise ValidationError("No se puede eliminar este periodo ya que posee una o mas guardias asociadas a el. Elimine estas relaciones para borrar el periodo.")

        elif len(self.per_liquidacion) > 0:
            raise ValidationError("No se puede eliminar este periodo ya que posee una o mas liquidaciones asociadas a el. Elimine estas relaciones para borrar el periodo.")
