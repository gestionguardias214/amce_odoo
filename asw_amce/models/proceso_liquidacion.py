...
class asw_proceso_liquidacion(models.Model):
    ...
    def cargar(self):
        guardias = self.env["asw.guardia"].search(
            [
                ("gua_profesional.pro_grupo", "in", self.prl_grupo.ids),
        ...
        self.write({"prl_profesional": [(6, 0, prof.ids)]})

    def validar(self):
        if self.prl_tipo == "masivo":
            if len(self.prl_profesional) == 0:
                raise ValidationError(
                    "Deben cargarse al menos un profesional para poder validar"
                )
            ...
            for prof in self.prl_profesional:
                ...
                conceptos = self.env["asw.profesional_concepto"].search(
                    [
                        ...
                        ("prc_periodo", "=", self.prl_periodo.id),
                    ]
                )

                self.generar_liquidacion(
                    guardias_prof, prof, self.prl_periodo, conceptos
                )
            ...
            if len(guardias) == 0:
                raise ValidationError(
                    "El profesional seleccionado no tiene guardias cargadas pendientes de liquidar para el rango de fechas"
                )

            periodo = self.obtenerPeriodo(
                date.today().strftime("%Y-%m-%d"), self.prl_pro_especifico.pro_grupo
            )
            ...
            self.generar_liquidacion(
                guardias, self.prl_pro_especifico, periodo, conceptos
            )
    ...
    def generar_liquidacion(self, guardias, profesinal, periodo, conceptos):
        ...
        guardias.write({"gua_liquidacion": liq.id, "state": "liquidada"})
        ...
        liq.write(
            {"liq_lineas": [(6, 0, lineas)], "liq_guardia": [(6, 0, guardias.ids)]}
        )

    def cancelar(self):
        if self.prl_periodo.get_esta_cerrado(self.prl_grupo):
            raise ValidationError(
                "No se puede cancelar la liquidacion ya que pertenece a un periodo cerrado"
            )
        else:
