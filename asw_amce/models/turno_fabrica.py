...
class asw_turno_fabrica(models.Model):
    ...
    def validar(self):
        linas_incompletas = self.tuf_lineas_turno_fabrica.filtered(lambda r: r.ltf_categoria.id == False or r.ltf_profesional.id == False)
        if(len(linas_incompletas) > 0):
            raise ValidationError("Existen linas que no se completaron, primero complete el profesional y la categoria y luego vuelva a intentarlo")
        ...
        for linea in self.tuf_lineas_turno_fabrica:            
            ...
            if linea.ltf_vianda != 0.0:
                ...
                descripcion_vianda = 'Turno Fabrica Nro %s por el concepto %s' % (self.id, concepto_vianda.descripcion)
            ...
            if linea.ltf_viatico != 0.0:
                descripcion_viatico = 'Turno Fabrica Nro %s por el concepto %s' % (self.id, concepto_viatico.descripcion)
                self.env['asw.profesional_concepto'].create({
    ...
    def validar_lineas(self):        
        ...
        if(len(lineas) > 0):
            ...
            raise ValidationError("No se puede validar este Turno de fabrica ya que los turnos %s tienen guardias generadas para los dias seleccionados" % (turnos.ids))
