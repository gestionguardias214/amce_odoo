...
class asw_profesionales(models.Model):
    ...
    _sql_constraints = {
        ('codigo_unique', 'UNIQUE(pro_codigo)', 'No puede haber dos profesionales con el mismo codigo'),
        # ('cbu_unique', 'UNIQUE(pro_cbu)', 'No puede haber dos profesionales con el mismo cbu'),   
    }
    
    partner_id = fields.Many2one(
        string=u'Partner Asociado',
    ...
    pro_vto_poliza = fields.Date(
        string=u'Vto poliza de mala praxis',
        default=fields.Date.context_today,
    )    
    
    pro_vto_examenes = fields.Date(
        string=u'Vto examenes periodicos',
        default=fields.Date.context_today,
    )

    #vto licencia
    pro_vto_licencia = fields.Date(
        string=u'Vto licencia de conducir',
    )

    prox_venc_lic = fields.Boolean(
        string=u'Licencia proxima a vencer',        
    ...
    prox_venc_exa = fields.Boolean(
        string=u'Examenes proximos a vencer',        
        compute='_compute_venc',
    ...
    prox_venc_pol = fields.Boolean(
        string=u'Poliza pronta a vencer',
        compute='_compute_venc',
    ...
    pro_fecha_alta = fields.Date(
        string=u'Fecha de Alta',        
    )
    
    pro_fecha_baja = fields.Date(
        string=u'Fecha de Baja',        
    )

    pro_fecha_ultimo_sac = fields.Date(
        string=u'Fecha de último SAC',        
    )
    ...
    def unlink(self):
        for record in self:
            if len(record.pro_guardia) > 0 and len(record.pro_liquidacion) > 0 and len(record.pro_concepto) > 0:
                raise ValidationError("No se puede eliminar este profesional ya que posee una o mas guardias, liquidaciones, y conceptos asociadas a el. Elimine estas relaciones para borrar el profesional.")

            elif len(record.pro_guardia) > 0 and len(record.pro_liquidacion) > 0:
                raise ValidationError("No se puede eliminar este profesional ya que posee una o mas guardias y liquidaciones asociadas a el. Elimine estas relaciones para borrar el profesional.")

            elif len(record.pro_guardia) > 0 and len(record.pro_concepto) > 0:
                raise ValidationError("No se puede eliminar este profesional ya que posee una o mas guardias y conceptos asociadas a el. Elimine estas relaciones para borrar el profesional.")

            elif len(record.pro_liquidacion) > 0 and len(record.pro_concepto) > 0:
                raise ValidationError("No se puede eliminar este profesional ya que posee una o mas liquidaciones, y conceptos asociadas a el. Elimine estas relaciones para borrar el profesional.")

            elif len(record.pro_guardia) > 0:
                raise ValidationError("No se puede eliminar este profesional ya que posee una o mas guardias asociadas a el. Elimine estas relaciones para borrar el profesional.")

            elif len(record.pro_liquidacion) > 0:
                raise ValidationError("No se puede eliminar este profesional ya que posee una o mas liquidaciones asociadas a el. Elimine estas relaciones para borrar el profesional.")

            elif len(record.pro_concepto) > 0:
                raise ValidationError("No se puede eliminar este profesional ya que posee uno o mas conceptos asociados a el. Elimine estas relaciones para borrar el profesional.")
