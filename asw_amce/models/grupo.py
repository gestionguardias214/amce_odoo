...
class asw_grupo(models.Model):
    ...
    _sql_constraints = [
        (
            "gru_codigo",
            "UNIQUE(gru_codigo)",
            "Ya se ha ingresado un grupo con ese codigo, por favor elija otro",
        ),
    ]
    ...
    gru_dia_inicio = fields.Integer(
        string="Día de Inicio de periodo",
        track_visibility="onchange",
    )

    gru_dia_inicio_especial = fields.Boolean(
        string="Inicia la guardia un día distinto al Primero",
    )
        ...
        for record in self:
            if len(record.gru_profesional) > 0 and len(record.gru_categoria) > 0:
                raise ValidationError(
                    "No se puede eliminar este grupo ya que posee uno o mas profesionales y categorias asociados a el. Elimine estas relaciones para borrar el grupo."
                )

            elif len(record.gru_profesional) > 0:
                raise ValidationError(
                    "No se puede eliminar este grupo ya que posee uno o mas profesionales asociados a el. Elimine estas relaciones para borrar el grupo."
                )

            elif len(record.gru_categoria) > 0:
                raise ValidationError(
                    "No se puede eliminar este grupo ya que posee una o mas categorias asociadas a el. Elimine estas relaciones para borrar el grupo."
                )
