...
class asw_actualizacion_precio(models.Model):
    ...
    acp_porcentaje = fields.Float(
        string=u'Porcentaje de actualizacion',
    )
    
    acp_linea_actualizacion = fields.One2many(
        string=u'Lineas de actualizacion',
    ...
    acp_pca_ids = fields.One2many(
        string=u'Precios de Categorias',
    ...
    def aumentar(self):
        for record in self:
            for linea in record.acp_linea_actualizacion:
                linea.lia_precio_nuevo += linea.lia_precio_actual * record.acp_porcentaje / 100
