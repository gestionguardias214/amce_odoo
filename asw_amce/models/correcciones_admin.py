...
class asw_correcciones_admin(models.Model):
    ...
    def asignar_grupo(self):
        profesionales = self.env['asw.profesionales'].search([('pro_grupo', '=', False)])      
    ...
    def recuardar_profesionales(self):        
        profesionales = self.env['asw.profesionales'].search([('partner_id', '=', False)])        
    ...
    def actualizar_precio_guardias_fabrica(self):
        guardias = self.env['asw.guardia'].search([('gua_tuf','!=', False),('gua_periodo', '=', self.periodo_id.id)])        
        for record in guardias:            
