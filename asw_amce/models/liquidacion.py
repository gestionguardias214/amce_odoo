...
class asw_liquidacion(models.Model):
    ...
    liq_cant_guardias = fields.Integer(
        string=u'Cant de guardias',
        compute='_compute_cant_guardias'
    )
    ...
    def calcular_monto_sac(self):
        for record in self:
            lineas_aplica = record.liq_lineas.filtered(lambda r: r.lin_concepto.con_sac == True)
    ...
    def unlink(self):
        for record in self:
            if record.liq_periodo.get_esta_cerrado(record.liq_grupo):
                raise ValidationError("No se puede eliminar esta liquidacion ya que el periodo al que pertenece esta cerrado.")
            else:            
    ...
    def write(self, vals):
        for record in self:
            if record.liq_periodo.get_esta_cerrado(record.liq_grupo):
                raise ValidationError("No se puede editar esta liquidacion ya que el periodo al que pertenece esta cerrado.")
