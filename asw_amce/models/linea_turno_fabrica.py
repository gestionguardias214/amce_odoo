...
class asw_linea_turno_fabrica(models.Model):
    ...
    ltf_fecha_fin = fields.Datetime(
        ...
        compute='_compute_ltf_fecha_inicio',
        store=True
    )
    ...
    def _compute_ltf_fecha_inicio(self):
        for record in self:
            ...
            fecha_inicio = fecha_inicio + timedelta(hours=horas) + timedelta(minutes=minutos)
            fecha_fin = fecha_inicio + timedelta(hours=int(record.ltf_turno.tur_canthoras))
    ...
    def _compute_cat(self):
        for record in self:
            if (record.ltf_profesional.id != False):            
                ...
                result['domain']={'ltf_categoria':[('id','in',ids),('cat_es_fabrica', '=', True)]}
    ...
    def _al_cambiar_fechas(self):
        for record in self:
            ...
            if(diferencia.total_seconds() <= 0):
                raise exceptions.ValidationError("La cantidad de horas entre la fecha de inicio y fin no puede ser inferior o igual a cero, por favor revise las fechas")
    ...
    def _compute_total(self):
        for record in self:
            record.ltf_total = record.ltf_importe + record.ltf_viatico + record.ltf_vianda