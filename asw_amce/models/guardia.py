...
class asw_guardia(models.Model):
    ...
    gua_egreso = fields.Datetime(
        string="Fecha y hora de Egreso",
    )

    gua_precioxhora = fields.Float(
        string="Precio por Hora", compute="_compute_precioxhora"
    )

    gua_canthoras = fields.Float(
        string="Cantidad de horas", compute="_compute_canthoras", store=True
    )
    ...
    gua_importe_conceptos = fields.Float(
        string="Importe Conceptos", compute="calcular_importe_conceptos", store=True
    )

    gua_importe_total = fields.Float(
        string="Importe Total", compute="calcular_importe_total", store=True
    )
    ...
    gua_editable = fields.Boolean(
        string="Editable", compute="_compute_gua_editable", default=True
    )
    ...
    def calcular_importe_conceptos(self):
        for record in self:
            record.gua_importe_conceptos = sum(
                record.gua_profesional_concepto.mapped("prc_importe")
            )
    ...
    def _compute_gua_editable(self):
        for record in self:
            if record.gua_periodo.id == False:
                ...
                record.gua_editable = not (
                    record.gua_periodo.per_cerrado == True
                    and record.gua_grupo.id in record.gua_periodo.per_gru_ids.ids
                )
    ...
    def write(self, vals):
        ...
        if len(self) == 1:
            ...
            liquidacion_actual = self.env["asw.liquidacion"].search(
                [
                    ("liq_profesional", "=", self.gua_profesional.id),
                    ("liq_periodo", "=", self.gua_periodo.id),
                ]
            )
    ...
    def create(self, vals):
        ...
        if "gua_periodo" in vals:
            liquidacion_actual = self.env["asw.liquidacion"].search(
                [
                    ("liq_profesional", "=", vals["gua_profesional"]),
                    ("liq_periodo", "=", vals["gua_periodo"]),
                ]
            )

            if len(liquidacion_actual) == 1:
                result.write(
                    {"gua_liquidacion": liquidacion_actual.id, "state": "liquidada"}
                )
    ...
    def _compute_canthoras(self):
        ...
        for record in self:
            if record.gua_egreso != False:
                record.gua_canthoras = record.calcular_horas(
                    record.gua_egreso, record.gua_ingreso
                )
                record.gua_periodo = record.obtenerPeriodo(
                    record.gua_egreso, record.gua_profesional.pro_grupo
                )
    ...
    def poner_periodo(self):
        self.gua_periodo = self.obtenerPeriodo(
            self.gua_egreso, self.gua_profesional.pro_grupo
        )

    @api.depends("gua_categoria", "gua_ingreso")
    def _compute_precioxhora(self):
        for record in self:
            record.gua_precioxhora = record.gua_categoria._compute_buscar_precio(
                record.gua_ingreso
            )
    ...
    def checkeo(self):
        for record in self:
            if (
                record.gua_periodo.get_esta_cerrado(record.gua_grupo) == True
                and record.state == "l"
            ):
                raise ValidationError(
                    "No se puede eliminar esta guardia ya que el periodo asociado a ella esta cerrado, y su estado es liquidada."
                )

            elif record.gua_periodo.get_esta_cerrado(record.gua_grupo) == True:
                raise ValidationError(
                    "No se puede eliminar esta guardia ya que el periodo asociado a ella esta cerrado."
                )

            elif record.state == "l":
                raise ValidationError(
                    "No se puede eliminar esta guardia ya que su estado es liquidada."
                )
