...
class asw_presentismo(models.Model):
    ...
    def agregar(self):
        ...
        if len(self.pre_grupo) == 0:
            raise ValidationError(
                "Debe seleccionar al menos un grupo para buscar profesionales"
            )

        guardias = self.env["asw.guardia"].search(
            [
                ("gua_periodo", "=", self.pre_periodo.id),
                ("gua_categoria.cat_grupo", "in", self.pre_grupo.ids),
        ...
        for prof in profesionales:
            self.pre_linea_presentismo.create(
                ({"lpr_profesional": prof.id, "lpr_presentismo": self.id})
            )

        return result

    def validar_duplicado(self):
        proceso_periodo = self.search(
            [("id", "!=", self.id), ("pre_periodo", "=", self.pre_periodo.id)]
        )
            ...
            for record in proceso_periodo:
                for grup in record.pre_grupo:
                    if grup.id in self.pre_grupo.ids:
                        raise ValidationError(
                            "No puede validarse este proceso ya que existe otro proceso para el mismo periodo y grupo, el proceso es el nro %s"
                            % (record.id)
                        )
        return True
    ...
    def validar(self):
        ...
        if len(self.pre_linea_presentismo) == 0:
            raise ValidationError(
                "No puede procesarse un proceso de presentismo sin profesionales seleccionados, por favor agregue primero los profesionales"
            )
                ...
                self.env["asw.profesional_concepto"].create(
                    {
                        ...
                        "prc_descripcion": "Presentismo de la categoria %s usando el porcentaje %s por el importe total de guardias %s$"
                        % parametros,
                        "prc_importe": pres,
    ...
    def checkeo(self):
        for record in self:
            if record.pre_periodo.get_esta_cerrado(record.pre_grupo):
                raise ValidationError(
                    "No puede cancelarse un Presentismo de un periodo que se encuentra cerrado. Por favor abra el periodo y vuelva a intentarlo"
                )
            ...
            for linea in record.pre_linea_presentismo:
                for prc in linea.lpr_profesional_concepto:
                    if prc.prc_periodo.get_esta_cerrado(record.pre_grupo):
                        raise ValidationError(
                            "No se puede cancelar este presentismo ya que posee conceptos de periodos ya cerrados"
                        )
                    prc.unlink()
    ...
    def unlink(self):
        ...
        for record in self:
            if self.pre_estado == "validado":
                raise ValidationError(
                    "No puede eliminarse un Presentismo que se encuentre Validado"
                )
