...
class asw_profesional_concepto(models.Model):
    ...
    prc_tipo = fields.Selection(
        string="Tipo", selection=[("otro", "Otro"), ("presentismo", "Presentismo")]
    )
    ...
    prc_posee_liquidacion = fields.Boolean(
        string="Posee liquidacion", compute="_compute_posee_liquidacion"
    )
        ...
        for record in self:
            if record.prc_posee_liquidacion:
                raise ValidationError(
                    "No se puede eliminar este concepto ya que posee una liquidacion relacionada."
                )
            else:
    ...
    def create(self, vals):
        if (
            self.env["asw.concepto"]
            .search([("id", "=", vals["prc_concepto"])])
            .con_negativo
        ):
        ...
        liquidacion_actual = self.env["asw.liquidacion"].search(
            [
                ("liq_profesional", "=", vals["prc_profesional"]),
                ("liq_periodo", "=", vals["prc_periodo"]),
            ]
        )
    ...
    def write(self, vals):
        ...
        if len(self) == 1:
            liquidacion_actual = self.env["asw.liquidacion"].search(
                [
                    ("liq_profesional", "=", self.prc_profesional.id),
                    ("liq_periodo", "=", self.prc_periodo.id),
                ]
            )
