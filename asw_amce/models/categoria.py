...
class asw_categoria(models.Model):
    ...
    _sql_constraints = [
        (
            "codigo_unique",
            "UNIQUE(cat_codigo)",
            "Ya se ha ingresado una categoria con ese codigo, por favor elija otro",
        ),
    ]
    _rec_name = "name"
    _order = "name"
    ...
    cat_es_fabrica = fields.Boolean(
        string="Se utiliza en Turnos de Fábrica?",
    )

    cat_fabrica = fields.Many2one(string="Fabrica", comodel_name="asw.fabrica")
    ...
    def _compute_buscar_precio(self, fecha):
        for record in self:
            precio = self.env["asw.precio_categoria"].search(
                [("pca_fecha", "<=", fecha), ("pca_categoria", "=", self.id)],
                order="pca_fecha desc",
                limit=1,
            )
    ...
    def unlink(self):
        for record in self:
            if (
                len(record.cat_profesional) > 0
                and len(record.cat_guardia.search([("state", "=", "a")])) > 0
            ):
                raise ValidationError(
                    "No se puede eliminar esta categoria ya que posee una o mas guardias y profesionales asociados a ella. Elimine estas relaciones para borrar la categoria."
                )

            elif len(record.cat_profesional) > 0:
                raise ValidationError(
                    "No se puede eliminar esta categoria ya que posee uno o mas profesionales asociados a ella. Elimine estas relaciones para borrar la categoria."
                )

            elif len(record.cat_guardia) > 0:
                raise ValidationError(
                    "No se puede eliminar esta categoria ya que posee una o mas guardias asociadas a ella. Elimine estas relaciones para borrar la categoria."
                )
