...
class asw_calculo_sac(models.Model):
        ...
        for record in self:
            if record.get_esta_cerrado(record.sac_grupo) != False:
                raise ValidationError(
                    "No se puede cancelar el calculo de SAC, ya que las liquidaciones generadas pertenecen a un periodo cerrado."
                )
            else:
    ...
    def calcular_periodo(self):
        for record in self:
            ...
            periodo = self.env["asw.periodo"].search(
                [("per_desde", "like", filtro), ("per_cerrado", "=", False)],
                order="id desc",
                limit=1,
            )
    ...
    def calcular_dias_trabajados(self, profesional):
        ...
        if profesional.pro_fecha_ultimo_sac == False:
            ...
            if finicio == False:
                raise ValidationError(
                    "El profesional %s no tiene definida fecha de ultimo SAC ni Fechad e ALTA".format(
    ...
    def validar(self):
        for record in self:
            ...
            liquidaciones = self.env["asw.liquidacion"].search(
                [
                    ("liq_periodo", "in", self.sac_periodo_ids.ids),
                    ("liq_profesional.pro_grupo", "in", record.sac_grupo.ids),
                ]
            )
            ...
            for prof in profesionales:
                if prof.id != False:
                    liquidaciones_profesional = liquidaciones.filtered(
                        lambda r: r.liq_profesional.id == prof.id
                    )

                    cnt_periodos = len(liquidaciones_profesional)

                    total_bruto = sum(
                        liquidaciones_profesional.mapped("liq_monto_aplicable_sac")
                    )
                    ...
                    liq = record.crear_liquidacion(
                        self.sac_periodo, concepto, total, prof
                    )
    ...
    def unlink(self):
        for record in self:
            if record.get_esta_cerrado(record.sac_grupo):
                raise ValidationError(
                    "No se puede eliminar el calculo de SAC, ya que las liquidaciones generadas pertenecen a un periodo cerrado."
                )
            else:
