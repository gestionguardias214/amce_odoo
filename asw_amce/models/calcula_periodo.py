...
class asw_calcula_periodo(models.AbstractModel):
    ...
    def obtenerPeriodo(self, fecha, grupo):
        ...
        if (
            self.env.user.has_group("asw_amce.module_category_asw_amce_administrador")
            == False
        ):
            if fecha_fin.date() > hoy:
                raise exceptions.ValidationError(
                    "No pueden cargarse registros con fecha mayor a la actual"
                )
        ...
        if periodo.id == False:
            raise exceptions.ValidationError(
                "No existe periodo generado para %s, por favor contacte al administrador para que genere el nuevo periodo"
                % (filtro)
            )
