...
class asw_precio_categoria(models.Model):
    ...
    pca_act_precio = fields.Many2one(
        string=u'Proceso de Actualización de precio',
        comodel_name='asw.actualizacion_precio',
        ondelete='set null',
    )    
    
    pca_guardias_emitidas = fields.Boolean(
        string=u'Posee Guardias emitidas',        
        compute='_compute_pca_guardias_emitidas'
    )
    ...
    def create(self, vals):
        ...
        if(float(precio) == 0):
            raise exceptions.ValidationError("No puede crearse un registro de Precio de Categoria con un Precio igual a 0")
    ...
    def get_guardias_registro(self):
        guardias = self.env['asw.guardia'].search([('gua_categoria', '=', self.pca_categoria.id),('gua_ingreso', '>=', self.pca_fecha)])
        return guardias
    ...
    def write(self, vals):
        ...
        if self.pca_periodo_cerrado:
                raise ValidationError("No se puede modificarse este precio categoria ya que pertenece a un periodo que se encuentra cerrado")
    ...
    def checkeo(self):
        for record in self:
            if len ( self.env['asw.guardia'].search([ ('gua_ingreso', '>', record.pca_fecha) ]) ):
                raise ValidationError("No se puede eliminar este precio categoria ya que existen guardias emitidas luego de su fecha.")
            else:
                return True
