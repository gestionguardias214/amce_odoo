...
class asw_provincia(models.Model):
    ...
    _sql_constraints = [
        ('codigo_unique', 'UNIQUE(codigo)', 'Ya se ha ingresado un Provincia con este codigo, por favor ingrese un codigo diferente'), 
    ]
    ...
    def unlink(self):
        for record in self:
            if(len(record.pro_localidad) > 0):
                raise exceptions.ValidationError("No se puede eliminar una Provincia con Localidades relacionadas, por favor eliminelas y vuelva a intentarlo")
