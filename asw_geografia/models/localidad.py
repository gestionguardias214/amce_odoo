...
class asw_localidad(models.Model):
    ...
    _sql_constraints = [
        (
            "codigo_unique",
            "UNIQUE(codigo)",
            "Ya se ha ingresado un Localidades con este codigo, por favor ingrese un codigo diferente",
        ),
    ]
        ...
        for record in self:
            ...
            if cnt > 0:
                raise exceptions.ValidationError(
                    "No se puede eliminar una localidad con zonas relacionadas, por favor eliminelas y vuelva a intentarlo"
                )
